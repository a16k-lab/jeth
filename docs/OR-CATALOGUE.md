...
LIVE-AUDIT NOTE (2026-07-17, HEAD `ebeb7d5`; L7a updated 2026-07-18 @ `1ced397`): every row below was
re-probed. The genuine miscompile-avoiding keeps: **B-21** (JETH900), **L6** (JETH429), **FUNCREF-PURE**
(JETH055), **REC-STRUCT-MEMLOCAL** (JETH495), plus the **L7(a)** and **TERN-STRUCT-ARR** RESIDUALS. 3 rows
were STALE (lifted earlier, rows never retired) and are struck/updated below: **L2-MOBILE** and **A-LIT-RESID**
fully LIFTED; **TERN-STRUCT-ARR** lifted except its mixed fixed/dyn-chain residual.
2026-07-18 (Approach-B aliasing lift, `1ced397` merged to main): **L7(a)'s DEAD-AFTER PURE-LITERAL subset is
now LIFTED** (an IR fold to the byte-identical inline form); only its ESCAPING subset still rejects JETH465.
This was step 1 of 3 of the inline-member-layout family; B-21 and L6 were deliberately NOT attempted (user
ruling) - unlike L7(a) they have NO bytecode-identical fold target, so their valuable subset needs a
runtime-only DEEP COPY (the highest-miscompile-risk codegen), not a safe fold.
Note REC-STRUCT-MEMLOCAL is ALSO a Group-A design ruling (JETH495) - it needs a whole feature
(runtime-recursive struct-copy codegen), not a lift. Every remaining reject here is either a proven
miscompile (B-21/L6, L7a's escaping subset) or needs a feature JETH lacks (FUNCREF-PURE needs funcref-type
mutability; REC-STRUCT needs recursive-copy codegen); the sole open lift CANDIDATE is the TERN mixed-chain
residual (proven byte-identical, kept conservative).
...
| TERN-STRUCT-ARR (RESIDUAL only) | UPDATED 2026-07-17: the DYN `In[]` ternary (let-bind AND return) was LIFTED at `790135b` (#10; aliasing study done - solc unifies to a memory ref with an ASYMMETRIC rule, mem arm ALIASED / storage arm DEEP-COPIED, and the ternary IS the copy-desugar; runtime-verified byte-identical, incl. `In[][]`/`In[][][]`). The ONLY residual is the MIXED fixed/dyn chain `Arr<In,2>[]` in a ternary (a dynamic outer with a FIXED static-struct level inside). | JETH074 | The mixed chain is the B-21 pointer-headed fixed-array family (a deeper `Arr<Arr<In,2>,2>[]` miscompiles - fixed at `682a71f`/kept-reject). Note: an agent PROVED the `Arr<In,2>[]` ternary byte-identical this session but it was KEPT conservatively to match main - a LIFT CANDIDATE on its own evidence, not a proven miscompile |
...
| ~~L2-MOBILE~~ RETIRED 2026-07-17 | ~~the CAST+BARE int mix in an array literal: `abi.encode([u256(1n), 2n])`, `[u8(1n), 300n]`~~ **LIFTED at `017799b` (#9): the mixed cast/bare array literal now self-types to solc's common type (mobile-seed from element 0 + commonType fold; ORDER-SENSITIVE, SMALLEST-FIT). Live-audited at HEAD `ebeb7d5`: `abi.encode([u8(1n), 300n])` ACCEPTS and is byte-identical to solc (run+decode MATCH). Row was left stale after the lift.** | ~~JETH213~~ | LIFTED - no longer rejects |
| FUNCREF-PURE | a DECLARED-pure function (`get b(v: u256): Pure<u256>` - the `Pure<T>` return marker; `@pure` is banned natively so this is the only spelling that reaches the gate) calling through a funcref whose SIGNATURE has a state-writing address-taken target elsewhere in the contract (dispatcher-set poisoning): `b` using `Fd.f` of sig `(u256)=>u256` rejects when `ord()` address-takes state-writing `linc/ldec` of the same sig. The residual is exactly the UNTRACKABLE-SOURCE fallback: a funcref from a struct field (`d.f`), a param, a storage round-trip or a call result poisons the var and falls back to the per-SIGNATURE union (`currentPtrCallSigs`); a TRACKED `let` local is already discriminated per-variable (analyzer.ts:499-520, W5D-2) and ACCEPTS | JETH055 | JETH funcref types carry NO mutability (solc's `function(...) pure returns(...)` pointer types do - solc needs TWO struct types to hold a pure vs a nonpayable pointer, which is precisely what JETH's single `(x: u256) => u256` spelling cannot express), so the purity checker soundly assumes the sig-key dispatcher set; a lift needs mutability in the funcref type grammar. Workarounds (both runtime-verified MATCH): (1) use a plain nonpayable method (drop the `get` + `Pure<T>` marker); (2) bind the funcref to a TRACKED `let` local (`let g: (x: u256) => u256 = this.dbl`) instead of an untrackable source. CORRECTED 2026-07-16: the row previously said "make the function non-pure (read state so it infers view)" - that DOES NOT WORK (probed: `View<u256>` -> JETH054, `External<u256>` on a `get` -> JETH043), because the poisoned set contains a WRITER so the caller is deemed state-MODIFYING and view is as invalid as pure |
| ~~A-LIT-RESID~~ RETIRED 2026-07-17 | ~~a whole calldata-param branch in a pointer-headed nested ternary `c ? p : [a, b]` (p: `Arr<u256[],N>` cd param)~~ **LIFTED at `790135b` (#10): the cd->mem copy is emitted LAZILY inside its own arm (proven by an oversized-INNER-length witness: reverts on the arm reading `p`, returns the literal arm's value where solc does). Live-audited at HEAD `ebeb7d5`: `let q: Arr<u256[],2> = c ? p : [a, b]` ACCEPTS. Row was left stale after the lift.** | ~~JETH074~~ | LIFTED - no longer rejects |
| REC-STRUCT-MEMLOCAL | a RECURSIVE struct (`type P = { x: u256; kids: P[] }`) as a MEMORY LOCAL (`let m: P = this.p`), an uninitialized/ctor local, or an internal `P memory` return. solc lowers `P memory m = p` to an UNBOUNDED RUNTIME-RECURSIVE DEEP COPY of the whole tree (witnessed on a populated 3-level tree: a pointer-headed image at 0x80..0x340 whose size depends on every level's runtime array length; mutating `m` leaves storage untouched). The reject-parity direction (`p = m`, memory->storage) solc ALSO rejects (legacy "Copying of type struct P memory[] memory to storage is not supported"). Pinned in test/lift-recursive-ref-struct.test.ts | JETH200 / JETH074 | JETH's recursive back-edge is a `recursiveRef` EMPTY-FIELDS sentinel (isStaticType forced false; isDynStructLeaf / isDynStructElemArrayField / isSupportedDynStructLocal all reject it), and JETH has no runtime-recursive struct-copy codegen. Admitting the local would lay out zero/one word per `kids` element and DROP the nested payload - the EXACT silent miscompile that reverted REC-STRUCT-CONSUMERS (a memory local IS a storage->memory aggregate copy, same hazard class). Lifting needs runtime-recursive copy codegen + gating every consumer; a clean reject beats the miscompile |
