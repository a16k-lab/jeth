...
2026-07-18 (Group-B-except-B21/L6 campaign, `af70dc7`+`62c6d50` merged to main): TWO MORE residuals LIFTED -
the **TERN mixed-chain** `Arr<In,N>[]` ternary (precise predicate widen, deeper chain kept) and the
**FUNCREF-PURE non-escaping-local-struct** subset (check-only tracking extension, ~48 escape probes 0 OA).
**REC-STRUCT was ATTEMPTED and BLOCKED** with a stronger reason than "hard": it is UNPROVABLE IN ISOLATION
(its populated-tree deep-copy source is UNCONSTRUCTIBLE in JETH - see the row). L7(a)'s escaping residual is
coupled to B-21 (needs the pointer-headed layout), so it was excluded.
Every remaining reject here is either a proven miscompile (B-21/L6, L7a's escaping subset, the TERN deeper
chain) or needs a feature JETH lacks that cannot be shipped in isolation (FUNCREF-PURE's untrackable sources
need funcref-type-mutability SYNTAX; REC-STRUCT needs recursive-copy codegen AND a storage-recursive-nav
subsystem). No open lift candidate remains in this table.
