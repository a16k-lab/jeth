...
LIVE-AUDIT NOTE (2026-07-17, HEAD `ebeb7d5`): every row below was re-probed. 5 STILL REJECT and are the
genuine miscompile-avoiding keeps: **B-21** (JETH900), **L6** (JETH429), **L7(a)** (JETH465),
**FUNCREF-PURE** (JETH055), **REC-STRUCT-MEMLOCAL** (JETH495). 3 rows were STALE (lifted earlier this
session, rows never retired) and are struck/updated below: **L2-MOBILE** and **A-LIT-RESID** fully LIFTED;
**TERN-STRUCT-ARR** lifted except its mixed fixed/dyn-chain residual. Note REC-STRUCT-MEMLOCAL is ALSO a
Group-A design ruling (JETH495) - it needs a whole feature (runtime-recursive struct-copy codegen), not a
lift. Every remaining reject here is either a proven miscompile (B-21/L6/L7a) or needs a feature JETH lacks
(FUNCREF-PURE needs funcref-type mutability; REC-STRUCT needs recursive-copy codegen); the sole lift
CANDIDATE is the TERN mixed-chain residual (proven byte-identical, kept conservative).
