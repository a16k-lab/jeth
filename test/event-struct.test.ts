...
describe('non-indexed struct / fixed-array event params vs Solidity', () => {
  it('a static struct param (inline in the data head)', async () => {
    await diffLogs(
      `type P = { a: u256; b: u256; }; class C { E: event<{ p: P }>; f(a: u256, b: u256): External<void> { emit(E(P(a, b))); } }`,
  ...
  it('a value param before a struct (mixed head offsets) + a packed struct', async () => {
    await diffLogs(
      `type P = { a: u8; b: u16; c: bool; }; class C { E: event<{ n: u256; p: P }>; f(n: u256, a: u8, b: u16, c: bool): External<void> { emit(E(n, P(a, b, c))); } }`,
  ...
  it('a struct param alongside a dynamic (bytes) param (struct inline, bytes in tail)', async () => {
    await diffLogs(
      `type P = { a: u256; b: u256; }; class C { E: event<{ p: P; data: bytes }>; f(a: u256, b: u256, d: bytes): External<void> { emit(E(P(a, b), d)); } }`,
  ...
  it('a non-indexed DYNAMIC struct (bytes/string/array fields) param', async () => {
    await diffLogs(
      `type D = { a: u256; s: string; }; class C { E: event<{ n: u256; d: D }>; f(n: u256, a: u256, s: string): External<void> { emit(E(n, D(a, s))); } }`,
    ...
    await diffLogs(
      `type D = { a: u256; bs: bytes; xs: u256[]; }; class C { E: event<{ d: D }>; f(a: u256, bs: bytes, xs: u256[]): External<void> { let m: D = D(a, bs, xs); emit(E(m)); } }`,
  ...
  it('a static fixed-array param and a nested static struct', async () => {
    await diffLogs(
      `class C { E: event<{ xs: Arr<u256,3> }>; f(a: u256, b: u256, c: u256): External<void> { emit(E([a, b, c])); } }`,
    ...
    await diffLogs(
      `type I = { x: u256; y: u256; }; type O = { i: I; z: u256; }; class C { E: event<{ o: O }>; f(x: u256, y: u256, z: u256): External<void> { emit(E(O(I(x, y), z))); } }`,
