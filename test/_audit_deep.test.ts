...
describe('struct with fixed-array-of-struct field', () => {
  ...
  it('s.ps[i].x / .y / s.id', async () => {
    ...
    for (const i of [0n, 1n]) {
      await eq(`spx i=${i}`, '0x' + functionSelector('spx((uint64,(uint128,uint128)[2]),uint256)') + [...sWords, W(i)].join(''));
      await eq(`spy i=${i}`, '0x' + functionSelector('spy((uint64,(uint128,uint128)[2]),uint256)') + [...sWords, W(i)].join(''));
    }
    await eq('spx OOB', '0x' + functionSelector('spx((uint64,(uint128,uint128)[2]),uint256)') + [...sWords, W(2n)].join(''));
    // dirty p1.x (read) -> revert; dirty p1.y unread when reading spx i=1
    const dirty = [...sWords]; dirty[3] = W((1n << 200n) | 0x20n); // p1.x dirty (uint128)
    await eq('spx i=1 dirty p1.x', '0x' + functionSelector('spx((uint64,(uint128,uint128)[2]),uint256)') + [...dirty, W(1n)].join(''));
    await eq('spy i=1 dirty p1.x unread', '0x' + functionSelector('spy((uint64,(uint128,uint128)[2]),uint256)') + [...dirty, W(1n)].join(''));
  });
});
...
describe('fixed-array-of-fixed-array-of-struct', () => {
  ...
  it('a[i][j].x / .y all + OOB', async () => {
    ...
    for (const i of [0n, 1n]) for (const j of [0n, 1n]) {
      await eq(`aax ${i},${j}`, '0x' + functionSelector(`aax(${t},uint256,uint256)`) + [...aWords, W(i), W(j)].join(''));
      await eq(`aay ${i},${j}`, '0x' + functionSelector(`aay(${t},uint256,uint256)`) + [...aWords, W(i), W(j)].join(''));
    }
