...
describe('parse-error rejection (BYTE-SLICE-MC + malformed-source robustness)', () => {
  it('the colon-slice byte-index x[s:e][j] rejects instead of silently compiling a truncated x[s]', () => {
    expect(rejects(`@contract class C { @external @pure f(x: bytes, j: u256): bytes1 { return x[2n:5n][j]; } }`)).toBe(true);
    // the slice alone rejects too (colon-slice is not JETH syntax; use .slice()).
    expect(rejects(`@contract class C { @external @pure f(x: bytes): bytes { return x[2n:5n]; } }`)).toBe(true);
    // the byte-identical form (.slice) compiles.
    expect(rejects(`@contract class C { @external @pure f(x: bytes, s: u256, e: u256): bytes { return x.slice(s, e); } }`)).toBe(false);
    expect(
      rejects(`@contract class C { @external @pure f(x: bytes, j: u256): bytes1 { let sl: bytes = x.slice(2n, 5n); return sl[j]; } }`),
    ).toBe(false);
  });

  it('other malformed sources reject instead of compiling an error-recovered AST', () => {
    expect(rejects(`@contract class C { @external @pure f(): u256 { return 1n + ; } }`)).toBe(true);
    expect(rejects(`@contract class C { @external @pure f(): u256 { let x = ; return 1n; } }`)).toBe(true);
  });
  ...
  it('deliberate TS error-recovery patterns are preserved', () => {
    ...
    expect(
      rejects(`@struct class D { a: u256; tags: bytes } @contract class C { @external @pure f(b: bytes): u256 { let ds: D[] = abi.decode(b, D[]); return ds.length; } }`),
    ).toBe(false);
    expect(
      rejects(`@contract class C { @external @pure f(b: bytes): u256 { let m: u256[][] = abi.decode(b, u256[][]); return m.length; } }`),
    ).toBe(false);
    // a shape the analyzer rejects semantically keeps its specific code (not the generic syntax code).
    expect(
      codeOf('@abstract class A { @state a: u256; constructor(v: u256){ this.a = v; } } @contract class C extends A(1n) { constructor() A(2n) { } }'),
    ).toContain('JETH379');
    // a valid ternary index (colon inside brackets, but with a `?`) still compiles.
    expect(
      rejects(`@contract class C { @external @pure f(a: u256[], c: bool): u256 { return a[c ? 0n : 1n]; } }`),
    ).toBe(false);
  });
});
