...
describe('@external struct-flattening getters vs Solidity', () => {
  it('value + packed fields', async () => {
    await diff(
      `@struct class P { a: u8; b: u16; c: u32; owner: address; flag: bool; x: u256; } @contract class C { @external @state p: P; @external set(): void { this.p.a = 200n; this.p.b = 50000n; this.p.c = 4000000000n; this.p.owner = msg.sender; this.p.flag = true; this.p.x = 42n; } }`,
  ...
  it('bytes/string fields (dynamic members in the returned tuple)', async () => {
    await diff(
      `@struct class P { x: u256; name: string; y: u256; data: bytes; } @contract class C { @external @state p: P; @external set(): void { this.p.x = 7n; this.p.name = "hi"; this.p.y = 9n; this.p.data = msg.data; } }`,
  ...
  it('omits array and mapping members (declaration order preserved)', async () => {
    await diff(
      `@struct class P { x: u256; arr: u256[]; y: u256; } @contract class C { @external @state p: P; @external set(): void { this.p.x = 11n; this.p.y = 22n; this.p.arr.push(5n); } }`,
      `struct P { uint256 x; uint256[] arr; uint256 y; } contract C { P public p; function set() external { p.x=11; p.y=22; p.arr.push(5); } }`,
      [{ sig: 'set()' }, { sig: 'p()' }],
    );
    await diff(
      `@struct class P { x: u256; m: mapping<u256,u256>; y: u256; } @contract class C { @external @state p: P; @external set(): void { this.p.x = 11n; this.p.y = 22n; this.p.m[3n] = 9n; } }`,
...
describe('@external struct getters reached via mapping/array + nested structs vs Solidity', () => {
  ...
  it('nested static struct is flattened inline (dynamic array + mapping members omitted)', async () => {
    await diff(
      `@struct class I { a: u256; b: address; } @struct class O { x: u256; inner: I; z: u256; } @contract class C { @external @state o: O; @external set(): void { this.o.x = 1n; this.o.inner.a = 2n; this.o.inner.b = address(0x333n); this.o.z = 4n; } }`,
...
describe('@external nested-array / fixed-array getters vs Solidity', () => {
  ...
  it('struct fixed-array member: TOP struct omits it; a NESTED struct includes it', async () => {
    ...
    await diff(
      `@struct class P { x: u256; arr: Arr<u256,3>; y: address; } @contract class C { @external @state p: P; @external set(): void { this.p.x = 1n; this.p.arr[0n] = 10n; this.p.y = address(0x111n); } }`,
    ...
    await diff(
      `@struct class I { a: u256; arr: Arr<u256,3>; } @struct class O { x: u256; inner: I; } @contract class C { @external @state o: O; @external set(): void { this.o.x = 1n; this.o.inner.a = 2n; this.o.inner.arr[0n] = 7n; this.o.inner.arr[2n] = 9n; } }`,
...
describe('@external getters with a nested DYNAMIC struct member vs Solidity', () => {
  it('struct var with a nested struct that has a string member', async () => {
    await diff(
      `@struct class Meta { name: string; ts: u256; } @struct class Item { id: u256; meta: Meta; active: bool; } @contract class C { @external @state item: Item; @external set(): void { this.item.id = 7n; this.item.meta.name = "alice"; this.item.meta.ts = 99n; this.item.active = true; } }`,
  ...
  it('nested struct with bytes + a dynamic array member', async () => {
    await diff(
      `@struct class I { data: bytes; xs: u256[]; } @struct class O { x: u256; inner: I; y: address; } @contract class C { @external @state o: O; @external set(): void { this.o.x = 1n; this.o.inner.data = msg.data; this.o.inner.xs.push(5n); this.o.inner.xs.push(6n); this.o.y = address(0xabn); } }`,
  ...
  it('triple-nested dynamic struct', async () => {
    await diff(
      `@struct class D3 { s: string; } @struct class D2 { m: u256; c: D3; } @struct class D1 { n: u256; b: D2; } @contract class C { @external @state a: D1; @external set(): void { this.a.n = 1n; this.a.b.m = 2n; this.a.b.c.s = "deep"; } }`,
