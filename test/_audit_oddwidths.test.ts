...
describe('odd-width fixed-array element validation', () => {
  it('int24 sign-ext, uint40, bytes3 each index', async () => {
    ...
    await eq(
      'pi dirty',
      '0x' + functionSelector('pi(int24[3],uint256)') + [(1n << 100n) | 5n, 0n, 0n, 0n].map(W).join(''),
    );
    await eq(
      'pu dirty',
      '0x' + functionSelector('pu(uint40[3],uint256)') + [(1n << 40n) | 5n, 0n, 0n, 0n].map(W).join(''),
    );
    await eq(
      'pb dirty',
      '0x' +
        functionSelector('pb(bytes3[3],uint256)') +
        [(BigInt('0xabcdef') << BigInt((32 - 3) * 8)) | 1n, 0n, 0n, 0n].map(W).join(''),
    );
  });
});
