...
describe('constructor / inheritance over-acceptances now reject like solc 0.8.35', () => {
  it('BUG1: base constructor arguments given twice is rejected (JETH379), across every provision variant', () => {
    ...
    expect(
      codes('abstract class A { a: u256; constructor(v: u256){ this.a = v; } } class C extends A(1n) { constructor() A(2n) { } }'),
    ).toContain('JETH379');
    // same value provided twice
    expect(
      codes('abstract class A { a: u256; constructor(v: u256){ this.a = v; } } class C extends A(5n) { constructor() A(5n) { } }'),
    ).toContain('JETH379');
    // address argument provided twice
    expect(
      codes('abstract class A { o: address; constructor(v: address){ this.o = v; } } class C extends A(address(0x11n)) { constructor() A(address(0x22n)) { } }'),
    ).toContain('JETH379');
    // multi-hop grandbase: A's args given via B's heritage AND via C's modifier-style clause
    expect(
      codes('abstract class A { a: u256; constructor(v: u256){ this.a = v; } } abstract class B extends A(1n) { } class C extends B { constructor() A(3n) { } }'),
    ).toContain('JETH379');
    // grandbase with an intermediate parameterized ctor: A(1n) via B, B(7n) via C heritage, A(3n) via C ctor
    expect(
      codes('abstract class A { a: u256; constructor(v: u256){ this.a = v; } } abstract class B extends A(1n) { b: u256; constructor(w: u256){ this.b = w; } } class C extends B(7n) { constructor() A(3n) { } }'),
    ).toContain('JETH379');
    // control: a single heritage provider must STILL compile (no over-rejection)
    expect(codes('abstract class A { a: u256; constructor(v: u256){ this.a = v; } } class C extends A(1n) { }')).toEqual([]);
  });
  ...
  it('BOUNDARY: modifier-style base args used ALONE stay a SOUND reject (not liftable), never miscompiled', () => {
    ...
    expect(
      codes('abstract class A { x: u256; constructor(v: u256){ this.x = v; } } class C extends A { constructor() A(7n) { } }'),
    ).toContain('JETH379');
    // a TYPED phantom (`A(v: u256)`) is treated as a real member, so the base is left without args -> reject
    expect(
      codes('abstract class A { x: u256; constructor(v: u256){ this.x = v; } } class C extends A { constructor() A(v: u256) { } }'),
    ).toContain('JETH379');
    // a no-arg phantom `A()` against a parameterized base -> reject (never accepted with a defaulted arg)
    expect(
      codes('abstract class A { x: u256; constructor(v: u256){ this.x = v; } } class C extends A { constructor() A() { } }'),
    ).toContain('JETH379');
    // control: the SAME program expressed via the supported heritage form compiles cleanly (the lift path)
    expect(codes('abstract class A { x: u256; constructor(v: u256){ this.x = v; } } class C extends A(7n) { constructor() { } }')).toEqual([]);
  });
  ...
  it('BUG2: @override that overrides nothing is rejected (JETH369) - no base, and base lacking the fn', () => {
    ...
    expect(
      codes('abstract class A { x: u256; } class C extends A { @override get f(): External<u256> { return 42n; } }'),
    ).toContain('JETH369');
  });
  ...
  it('BUG3: msg.value in a non-payable base constructor is rejected (JETH162), even with a payable derived ctor', () => {
    ...
    expect(
      codes('abstract class A { v: u256; constructor(){ this.v = msg.value; } } abstract class B extends A { } class C extends B { @payable constructor(){ } }'),
    ).toContain('JETH162');
  });
  ...
  it('valid inheritance / constructor / override / payable patterns still compile (no over-rejection)', () => {
    ...
    expect(codes('abstract class A { @virtual f(): u256 { return 1n; } } class C extends A { @override f(): u256 { return 2n; } }')).toEqual([]);
    // payable base + payable derived reading msg.value
    expect(codes('abstract class A { v: u256; @payable constructor(){ this.v = msg.value; } } class C extends A { @payable constructor(){ } }')).toEqual([]);
    // msg.sender (NOT payability-gated) in a non-payable base + payable derived
    expect(codes('abstract class A { o: address; constructor(){ this.o = msg.sender; } } class C extends A { @payable constructor(){ } }')).toEqual([]);
    // a single payable constructor reading msg.value
    expect(codes('class C { v: u256; @payable constructor(){ this.v = msg.value; } }')).toEqual([]);
    // a 3-level chain with base args
    expect(codes('abstract class A { a: u256; constructor(x: u256){ this.a = x; } } abstract class B extends A(1n) { b: u256; constructor(y: u256){ this.b = y; } } class C extends B(2n) { constructor() { } }')).toEqual([]);
    // a real method whose name matches a base but is a genuine function (typed params) is not mistaken for base args
    expect(codes('abstract class A { a: u256; constructor(v: u256){ this.a = v; } get A(z: u256): External<u256> { return z; } } class C extends A(1n) { }')).toEqual([]);
    // same-name / different-signature botched override still rejects (unchanged)
    expect(codes('abstract class A { @virtual g(x: u256): u256 { return x; } } class C extends A { @override g(): u256 { return 1n; } }').length).toBeGreaterThan(0);
  });
});
