...
describe('dynamic-field struct -> storage assignment vs Solidity', () => {
  ...
  it('overwrite long->short string (old tail cleared) + packed value fields', async () => {
    await diff(
      ...
      [
        {
          sig: 'set(uint8,uint16,bytes)',
          args: W(200n) + W(5000n) + W(0x60n) + W(40n) + 'ab'.repeat(40).padEnd(128, '0'),
        },
  ...
  it('a struct with a dynamic value-array field, from a memory source (overwrite + mixed fields)', async () => {
    await diff(
      ...
      [
        {
          sig: 'set(uint8,string,uint256[])',
          args:
            W(200n) + W(0x60n) + W(0xa0n) + W(3n) + '616263'.padEnd(64, '0') + W(4n) + W(1n) + W(2n) + W(3n) + W(4n),
        },
        {
          sig: 'set(uint8,string,uint256[])',
          args: W(7n) + W(0x60n) + W(0xa0n) + W(2n) + '7a7a'.padEnd(64, '0') + W(2n) + W(99n) + W(88n),
        },
        { sig: 'geta()' },
        { sig: 'gets()' },
        { sig: 'len()' },
