...
describe('Cat A: bind a pointer-headed aggregate-array element (alias) - byte-identical to solc 0.8.35', () => {
  it('bytes[][]: let row = xs[i], for-of, nested for-of, empty loop, OOB bind', async () => {
    const J = `class C {
      get rowlen(): External<u256> { let xs: bytes[][] = [[bytes("a"), bytes("bbbb")]]; let row: bytes[] = xs[0n]; return row[1n].length; }
      get sumrows(): External<u256> { let xs: bytes[][] = [[bytes("a"), bytes("bb")], [bytes("ccc")]]; let n: u256 = 0n; for (const row of xs) { n = n + row.length; } return n; }
      get sumleaf(): External<u256> { let xs: bytes[][] = [[bytes("ab"), bytes("cde")]]; let n: u256 = 0n; for (const row of xs) { let k: u256 = 0n; while (k < row.length) { n = n + row[k].length; k = k + 1n; } } return n; }
      get emptyloop(): External<u256> { let xs: bytes[][] = []; let n: u256 = 0n; for (const row of xs) { n = n + row.length; } return n; }
      get oob(): External<u256> { let xs: bytes[][] = [[bytes("a")]]; let row: bytes[] = xs[5n]; return row.length; } }`;
    const S = `contract C {
  ...
  it('B3 dyn-struct D[]: let d = ps[i] reads + alias write-through + re-point + impure index + OOB', async () => {
    ...
    class C {
      get read(): External<u256> { let ps: P[] = [P(7n, bytes("xy")), P(9n, bytes("zzz"))]; let d: P = ps[1n]; return d.a + d.s.length; }
      get aliasw(): External<u256> { let ps: P[] = [P(7n, bytes("xy"))]; let d: P = ps[0n]; d.a = 5n; return ps[0n].a; }
      get repoint(): External<u256> { let ps: P[] = [P(7n, bytes("xy"))]; let d: P = ps[0n]; ps[0n] = P(99n, bytes("q")); return d.a; }
      c: u256;
      bump(): u256 { this.c = this.c + 1n; return 0n; }
      once(): External<u256> { let ps: P[] = [P(7n, bytes("xy"))]; let d: P = ps[this.bump()]; return this.c; }
      get oob(): External<u256> { let ps: P[] = [P(7n, bytes("xy"))]; let d: P = ps[5n]; return d.a; }
      get forof(): External<u256> { let ps: P[] = [P(1n, bytes("a")), P(2n, bytes("bb"))]; let n: u256 = 0n; for (const d of ps) { n = n + d.a + d.s.length; } return n; } }`;
  ...
  it('binding a static-struct array element now ACCEPTS (static-struct arrays are pointer-headed)', () => {
    ...
    expect(codes(`class C { get f(): External<u256> { let xs: bytes[][] = [[bytes("a")]]; let row: bytes[] = xs[0n]; return row.length; } }`)).toEqual([]);
    expect(codes(`type P = {a:u256;s:bytes;}; class C { get f(): External<u256> { let ps: P[] = [P(1n,bytes("x"))]; let d: P = ps[0n]; return d.a; } }`)).toEqual([]);
  });
});
