...
describe('Cat A2: pointer-headed aggregate-array element aliasing assignment - byte-identical to solc', () => {
  it('B3 dyn-struct P[]: xs[i] = xs[j] / = local aliases (e2e + re-point + local + whole-encode)', async () => {
    ...
    class C {
      get e2e(): External<u256> { let xs: P[] = new Array<P>(2n); xs[0n] = P(1n, bytes("x")); xs[1n] = xs[0n]; xs[0n].a = 5n; return xs[1n].a; }
      get repoint(): External<u256> { let xs: P[] = new Array<P>(2n); xs[0n] = P(1n, bytes("x")); xs[1n] = xs[0n]; xs[0n] = P(9n, bytes("q")); return xs[1n].a; }
      get fromlocal(): External<u256> { let p: P = P(3n, bytes("z")); let xs: P[] = new Array<P>(1n); xs[0n] = p; p.a = 7n; return xs[0n].a; }
      get enc(): External<bytes> { let xs: P[] = new Array<P>(2n); xs[0n] = P(1n, bytes("xy")); xs[1n] = xs[0n]; xs[0n].a = 5n; return abi.encode(xs); } }`;
  ...
  it('nested arrays bytes[][] / u256[][]: xs[i] = xs[j] aliases (e2e + whole-encode), literal/local still work', async () => {
    const J = `class C {
      get be2e(): External<u256> { let xs: bytes[][] = [[bytes("a")], [bytes("zz")]]; xs[1n] = xs[0n]; xs[0n][0n] = bytes("ABCDE"); return xs[1n][0n].length; }
      get benc(): External<bytes> { let xs: bytes[][] = [[bytes("aa"), bytes("b")], [bytes("zz")]]; xs[1n] = xs[0n]; return abi.encode(xs); }
      get ue2e(): External<u256> { let xs: u256[][] = [[1n, 2n], [9n]]; xs[1n] = xs[0n]; xs[0n][0n] = 77n; return xs[1n][0n]; }
      get blocal(): External<u256> { let xs: bytes[][] = [[bytes("a")]]; let r: bytes[] = [bytes("qqq")]; xs[0n] = r; return xs[0n][0n].length; }
      get blit(): External<bytes> { let xs: bytes[][] = [[bytes("a")]]; xs[0n] = [bytes("q"), bytes("rr")]; return abi.encode(xs); } }`;
    const S = `contract C {
  ...
  it('a static-struct array element ref-assign now ACCEPTS (static-struct arrays are pointer-headed)', () => {
    ...
    expect(codes(`type P = {a:u256;s:bytes;}; class C { get f(): External<u256> { let xs: P[] = new Array<P>(2n); xs[0n]=P(1n,bytes("x")); xs[1n]=xs[0n]; return xs[1n].a; } }`)).toEqual([]);
  });
});
