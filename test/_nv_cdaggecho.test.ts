...
const call = (sig: string, words: bigint[], extra = '') =>
  '0x' + sel(sig) + words.map(w).join('') + extra;
// left-aligned bytesN word from a bigint of the low bytes (n bytes)
const bytesNword = (loBytes: bigint, n: number) =>
  ((loBytes % (1n << BigInt(8 * n))) << BigInt(8 * (32 - n))) % M;
...
describe('probe', () => {
  ...
  it('runs', async () => {
    ...
    await eq('echo4 clean', call('echo4(uint256[2][2][2][2])', Array.from({ length: 16 }, (_, i) => BigInt(i) + 100n)));
    await eq('echo4 dirty(noop,u256 no mask)', call('echo4(uint256[2][2][2][2])', Array.from({ length: 16 }, () => DIRTY)));
    ...
    await eq('echoU8 maxbyte', call('echoU8(uint8[4][3])', u8clean.map(() => 0xffn)));
    await eq('echoI64 clean', call('echoI64(int64[5])', [0n, 1n, 2n, 3n, 4n]));
    await eq('echoI64 neg', call('echoI64(int64[5])', [-1n, -2n, (1n << 63n) - 1n, -(1n << 63n), 0n].map(x => ((x % M) + M) % M)));
    await eq('echoB4 clean', call('echoB4(bytes4[3])', [bytesNword(0x11223344n, 4), bytesNword(0xaabbccddn, 4), bytesNword(0xdeadbeefn, 4)]));
    ...
    for (let i = 0; i < 12; i++) {
      const d = [...u8clean]; d[i] = (DIRTY << 8n) | 0x42n; // high bits set, low byte 0x42
      await eq('echoU8 dirty[' + i + ']', call('echoU8(uint8[4][3])', d));
    }
    await eq('echoU8 all-dirty', call('echoU8(uint8[4][3])', u8clean.map((_, i) => (MAX << 8n) | BigInt(i))));
    ...
    await eq('echoStruct max-ok', call('echoStruct((uint256,uint8,address))', [MAX, 0xffn, BigInt('0x' + 'ff'.repeat(20))]));
    await eq('echoStructArr clean', call('echoStructArr((uint256,uint8,address)[2])', [1n, 2n, 0xaaaan, 3n, 4n, 0xbbbbn]));
    ...
    const mixedNeg = [...mixedClean]; mixedNeg[5] = ((-5n % M) + M) % M; // i40 negative (sign-extended to 256)
    await eq('echoMixed negF', call('echoMixed((uint128,uint64,bool,address,bytes8,int40))', mixedNeg));
    const mixedMax = [(1n << 128n) - 1n, (1n << 64n) - 1n, 1n, BigInt('0x' + 'ff'.repeat(20)), bytesNword((1n << 64n) - 1n, 8), (1n << 39n) - 1n];
    await eq('echoMixed maxF', call('echoMixed((uint128,uint64,bool,address,bytes8,int40))', mixedMax));
    const mixedMinI40 = [...mixedClean]; mixedMinI40[5] = ((-(1n << 39n) % M) + M) % M;
    await eq('echoMixed minI40', call('echoMixed((uint128,uint64,bool,address,bytes8,int40))', mixedMinI40));
    await eq('echoMixedArr clean', call('echoMixedArr((uint128,uint64,bool,address,bytes8,int40)[2])', [...mixedClean, ...mixedNeg]));
    ...
    const dirtyField = (idx: number, sig: string, base: bigint[], dval: bigint, label: string) => {
      const d = [...base]; d[idx] = dval; return eq(label, call(sig, d));
    };
    ...
    await dirtyField(3, 'echoMixed' + MSIG, mixedClean, (1n << 160n) | BigInt('0x' + 'a1'.repeat(20)), 'echoMixed dirty d(addr)');
    await dirtyField(4, 'echoMixed' + MSIG, mixedClean, bytesNword(0xdeadbeefcafef00dn, 8) | 0xffn, 'echoMixed dirty e(bytes8 low)');
    ...
    await dirtyField(5, 'echoMixed' + MSIG, mixedClean, MAX ^ ((1n << 39n) - 1n) | (1n << 39n), 'echoMixed f(i40 neg properly ext)');

    // struct array dirty element field
    await eq('echoStructArr dirty[1].b', call('echoStructArr((uint256,uint8,address)[2])', [1n, 2n, 0xaaaan, 3n, (1n << 8n) | 4n, 0xbbbbn]));
    await eq('echoStructArr dirty[0].c', call('echoStructArr((uint256,uint8,address)[2])', [1n, 2n, (1n << 161n) | 0xaaaan, 3n, 4n, 0xbbbbn]));
    ...
    await eq('echoWithArrArr clean', call('echoWithArrArr((uint64,uint256[3],bytes4)[2])', [...waClean, 0xaan, 0xbbn, 0xccn, 0xddn, bytesNword(0xdeadn, 4)]));
    // dirty id(u64) field -> validates -> revert
    await eq('echoWithArr dirty id', call('echoWithArr((uint64,uint256[3],bytes4))', [(1n << 64n) | 0x99n, 0x11n, 0x22n, 0x33n, bytesNword(0xcafebaben, 4)]));
    // dirty bytes4 tag low bytes -> validates -> revert
    await eq('echoWithArr dirty tag', call('echoWithArr((uint64,uint256[3],bytes4))', [0x99n, 0x11n, 0x22n, 0x33n, bytesNword(0xcafebaben, 4) | 0x1n]));
    // data leaves are u256 -> full width, can't be dirty; sanity max
    await eq('echoWithArr data=max', call('echoWithArr((uint64,uint256[3],bytes4))', [0x99n, MAX, MAX, MAX, bytesNword(0xcafebaben, 4)]));
    ...
    await eq('echoMixed trunc(5w)', call('echoMixed((uint128,uint64,bool,address,bytes8,int40))', mixedClean.slice(0, 5)));
    ...
    const i40min = -(1n << 39n), i40max = (1n << 39n) - 1n;
    ...
    await eq('echoMixed i40 inv +highbit', call('echoMixed' + MSIG, (() => { const d = [...mixedClean]; d[5] = (1n << 40n) | 0x10n; return d; })()));
    // value with bit39 set (negative) but bits 40..255 zero (should be all ones)
    await eq('echoMixed i40 inv bit39 no-ext', call('echoMixed' + MSIG, (() => { const d = [...mixedClean]; d[5] = (1n << 39n) | 0x3n; return d; })()));
    // bit39 clear (positive) but bits 40..255 all ones
    await eq('echoMixed i40 inv pos allones', call('echoMixed' + MSIG, (() => { const d = [...mixedClean]; d[5] = (M - 1n) ^ ((1n << 40n) - 1n); return d; })()));
    // top bit only set
    await eq('echoMixed i40 inv topbit', call('echoMixed' + MSIG, (() => { const d = [...mixedClean]; d[5] = 1n << 255n; return d; })()));
    ...
    await eq('echoB4 all dirtylow', call('echoB4(bytes4[3])', [
    ...
    await eq('echoMixed a u128 exact-max ok', call('echoMixed' + MSIG, (() => { const d = [...mixedClean]; d[0] = (1n << 128n) - 1n; return d; })()));
    await eq('echoMixed a u128 +1bit bad', call('echoMixed' + MSIG, (() => { const d = [...mixedClean]; d[0] = 1n << 128n; return d; })()));
    await eq('echoMixed b u64 exact-max ok', call('echoMixed' + MSIG, (() => { const d = [...mixedClean]; d[1] = (1n << 64n) - 1n; return d; })()));
    await eq('echoMixed b u64 +1bit bad', call('echoMixed' + MSIG, (() => { const d = [...mixedClean]; d[1] = 1n << 64n; return d; })()));

    // ==== WAVE 2: struct-array element-field validation at multiple positions ====
    await eq('echoMixedArr dirty[0].c bool', call('echoMixedArr' + MARR, (() => { const d = [...mixedClean, ...mixedClean]; d[2] = 5n; return d; })()));
    await eq('echoMixedArr dirty[1].f i40', call('echoMixedArr' + MARR, (() => { const d = [...mixedClean, ...mixedClean]; d[6 + 5] = (1n << 40n) | 1n; return d; })()));
    await eq('echoMixedArr dirty[1].e bytes8', call('echoMixedArr' + MARR, (() => { const d = [...mixedClean, ...mixedClean]; d[6 + 4] = bytesNword(0xdeadbeefcafef00dn, 8) | 0x7n; return d; })()));
    ...
    await eq('echoWithArrArr dirty[1].id', call('echoWithArrArr((uint64,uint256[3],bytes4)[2])', [...waClean, (1n << 64n) | 0xaan, 0xbbn, 0xccn, 0xddn, bytesNword(0xdeadn, 4)]));
    await eq('echoWithArrArr dirty[0].tag', call('echoWithArrArr((uint64,uint256[3],bytes4)[2])', [0x99n, 0x11n, 0x22n, 0x33n, bytesNword(0xcafebaben, 4) | 0xffn, 0xaan, 0xbbn, 0xccn, 0xddn, bytesNword(0xdeadn, 4)]));
    await eq('echoWithArrArr both-clean-max', call('echoWithArrArr((uint64,uint256[3],bytes4)[2])', [(1n << 64n) - 1n, MAX, MAX, MAX, bytesNword(0xffffffffn, 4), 0n, 0n, 0n, 0n, bytesNword(0n, 4)]));
