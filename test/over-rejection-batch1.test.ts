...
describe('FIX-ALL batch 1: pure-analyzer over-rejection lifts vs solc 0.8.35', () => {
  it('OR6: internal call returning bytes[] / string[] / P[]', async () => {
    await diff(
      `class C { mk(): bytes[] { let m: bytes[] = new Array<bytes>(2n); m[0n]=abi.encode(7n); m[1n]=abi.encode(8n); return m; } get get(): External<bytes[]> { return this.mk(); } }`,
    ...
    await diff(
      `type P = {a:u256;b:u256;}; class C { mk(): P[] { let m: P[] = [P(1n,2n),P(3n,4n)]; return m; } get get(): External<P[]> { return this.mk(); } }`,
  ...
  it('OR11: qualified internal-library tuple return (return / destructure / tuple-assign / 3-tuple)', async () => {
    const L = 'static class L { swap(a: u256, b: u256): [u256, u256] { return [b, a]; } }';
    const SL =
      'library L { function swap(uint256 a, uint256 b) internal pure returns (uint256, uint256){ return (b, a); } }';
    await diff(
      `${L} class C { get f(a: u256, b: u256): External<[u256, u256]> { return L.swap(a, b); } }`,
    ...
    await diff(
      `${L} class C { get f(a: u256, b: u256): External<u256> { let [x, y]: [u256,u256] = L.swap(a, b); return x*100n + y; } }`,
      `${SL} contract C { function f(uint256 a, uint256 b) external pure returns (uint256){ (uint256 x, uint256 y) = L.swap(a,b); return x*100+y; } }`,
      [['f(uint256,uint256)', W(7n) + W(9n)]],
    );
    await diff(
      `${L} class C { get f(a: u256, b: u256): External<u256> { let x: u256 = 0n; let y: u256 = 0n; [x, y] = L.swap(a, b); return x*100n + y; } }`,
    ...
    await diff(
      `static class L { tri(a:u256): [u256,u256,u256] { return [a, a*2n, a*3n]; } } class C { get f(a: u256): External<u256> { let [x,y,z]: [u256,u256,u256] = L.tri(a); return x+y+z; } }`,
  ...
  it('OR17: abi.encodeCall(IFoo.bar, [args]) - single, multi, dynamic-arg', async () => {
    await diff(
      `interface I { foo(x: u256): u256; } class C { get k(): External<bytes> { return abi.encodeCall(I.foo, [5n]); } }`,
    ...
    await diff(
      `interface I { foo(x: u256, a: address): bytes32; } class C { get k(a: address): External<bytes> { return abi.encodeCall(I.foo, [5n, a]); } }`,
    ...
    await diff(
      `interface I { foo(s: string, n: u256): bool; } class C { get k(): External<bytes> { return abi.encodeCall(I.foo, ["hi", 7n]); } }`,
  ...
  it('OR18: type(C).name and type(IFoo).name', async () => {
    await diff(
      `class C { get k(): External<string> { return type(C).name; } }`,
      `contract C { function k() external pure returns(string memory){ return type(C).name; } }`,
      [['k()', '']],
    );
    await diff(
      `interface IFoo { foo(x: u256): u256; } class C { get k(): External<string> { return type(IFoo).name; } }`,
