...
describe('native mutability inference (item #8)', () => {
  it('infers view / pure / nonpayable from the body (incl. transitively through internal calls)', () => {
    expect(mut(`@contract class C {
      @state x: u256;
      @external readsX(): u256 { return this.x; }
      @external pureFn(a: u256): u256 { return a + 1n; }
      @external writesX(v: u256): void { this.x = v; }
      @external readsEnv(): address { return msg.sender; }
      @external viaHelper(): u256 { return this.helper(); }
  ...
  it('inferred == explicit @view/@pure bytecode; explicit @payable / @view untouched', () => {
    expect(bc(`@contract class C { @state x: u256; @external f(): u256 { return this.x; } }`))
      .toBe(bc(`@contract class C { @state x: u256; @external @view f(): u256 { return this.x; } }`));
    expect(bc(`@contract class C { @external g(a: u256): u256 { return a + 1n; } }`))
      .toBe(bc(`@contract class C { @external @pure g(a: u256): u256 { return a + 1n; } }`));
    expect(mut(`@contract class C { @state x: u256; @external @payable pay(): void {} @external @view v(): u256 { return this.x; } }`))
  ...
  it('the inferred contract runs byte-identical to solc with explicit mutability', async () => {
    const J = `@contract class C {
      @state x: u256;
      @external set(v: u256): void { this.x = v; }
      @external get(): u256 { return this.x; }
      @external calc(a: u256): u256 { return a * 2n; } }`;
    const S = `contract C {
      uint256 x;
...
describe('`get` accessors (item #8; `get` = read-only at ANY visibility, External<T> = the ABI form)', () => {
  ...
  it('rejects a writing `get`, a setter, and `get` in decorator mode', () => {
    expect(codes(`@contract class C { @state x: u256; get bad(): u256 { this.x = 1n; return this.x; } }`)).toContain('JETH043');
    expect(codes(`@contract class C { @state x: u256; set val(v: u256) { this.x = v; } }`)).toContain('JETH043');
...
describe('mutability compatibility (item #8 sweep fixes)', () => {
  ...
  it('cluster 1: the override mutability ladder is checked with the RESOLVED (inferred) mutability', () => {
    ...
    expect(codes(`${base('@view')} @contract class C extends A { @override @external f(): u256 { this.x = 1n; return this.x; } }`)).toContain('JETH378');
    // view base (inferred) + writing override -> reject
    expect(codes(`@abstract class A { @state x: u256; @virtual @external f(): u256 { return this.x; } } @contract class C extends A { @override @external f(): u256 { this.x = 1n; return this.x; } }`)).toContain('JETH378');
    // explicit view base + native read-only override (inferred view) -> ACCEPT (was spuriously rejected pre-fix)
    expect(codes(`${base('@view')} @contract class C extends A { @override @external f(): u256 { return this.x + 1n; } }`)).toEqual([]);
  });

  it('cluster 2: an `implements` clause is rejected (JETH391); interfaces are implemented via `extends`', () => {
    expect(codes(`@interface class I { @external @pure f(): u256; } @contract class C implements I { @external @view f(): u256 { return 0n; } }`)).toContain('JETH391');
    expect(codes(`interface I { m(): View<u256>; } class C implements I { @state x: u256; @external m(): u256 { this.x = 1n; return this.x; } }`)).toContain('JETH391');
