...
describe('TERN-STRUCT-ARR return lift - byte-identical to solc 0.8.35', () => {
  it('return c ? m : this.fa (mem|storage) and c ? m : n (mem|mem)', async () => {
    const J = `${IN}
      class C {
        ...
        get retT(c: bool): External<Arr<In,2>> { let m: Arr<In,2> = [In(1n,2n),In(3n,4n)]; return c ? m : this.fa; }
        get retMM(c: bool): External<Arr<In,2>> { let m: Arr<In,2> = [In(1n,2n),In(3n,4n)]; let n: Arr<In,2> = [In(5n,6n),In(7n,8n)]; return c ? m : n; } }`;
  ...
  it('read-only consumers lift (return + arg-0 abi.encode); mutation-sensitive / non-first-arg / abiDecode-branch stay sound rejects', () => {
    ...
    expect(rejects(`${IN} class C { r(c: bool): External<Arr<In,2>> { let xs: Arr<Arr<In,2>,2> = [[In(1n,2n),In(3n,4n)],[In(5n,6n),In(7n,8n)]]; let m: Arr<In,2> = [In(9n,9n),In(9n,9n)]; return c ? xs[0n] : m; } }`)).toBe(true);
  });
  ...
  it('internal-call arg 0 of a ternary struct array - byte-identical incl. W1 field-write / W2 re-point / W3 cross-alias', async () => {
    const J = `${IN}
      class C {
        ...
        get w1(c: bool): External<u256> { let m: Arr<In,2>=[In(1n,2n),In(3n,4n)]; let n: Arr<In,2>=[In(5n,6n),In(7n,8n)]; this.mut(c ? m : n); return m[0n].a*1000n + n[0n].a; }
        get w2(c: bool): External<u256> { let m: Arr<In,2>=[In(1n,2n),In(3n,4n)]; let n: Arr<In,2>=[In(5n,6n),In(7n,8n)]; this.rep(c ? m : n); return m[1n].a*1000n + n[1n].a; }
        get w3(c: bool): External<u256> { let m: Arr<In,2>=[In(1n,2n),In(3n,4n)]; let n: Arr<In,2>=[In(5n,6n),In(7n,8n)]; return this.g3(c ? m : n, m); } }`;
    const S = `struct In { uint256 a; uint256 b; }
      contract C {
