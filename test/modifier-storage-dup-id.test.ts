...
describe('W8B: @modifier cross-kind duplicate identifiers (same contract)', () => {
  it('modifier vs @state -> both reject', () =>
    bothReject(
      `@contract class C { @state x: u256; @modifier x() { _; } @external f(): u256 { return this.x; } }`,
      `contract C { uint256 x; modifier x() { _; } function f() public view returns (uint256){ return x; } }`,
    ));
  it('modifier vs @constant -> both reject', () =>
    bothReject(
      `@contract class C { @constant x: u256 = 5n; @modifier x() { _; } }`,
      `contract C { uint256 constant x = 5; modifier x() { _; } }`,
    ));
  it('modifier vs @immutable -> both reject', () =>
    bothReject(
      `@contract class C { @immutable x: u256; @modifier x() { _; } constructor(){ this.x = 1n; } }`,
      `contract C { uint256 immutable x; modifier x() { _; } constructor(){ x = 1; } }`,
    ));
  it('modifier vs @event -> both reject', () =>
    bothReject(`@contract class C { @event x(); @modifier x() { _; } }`, `contract C { event x(); modifier x() { _; } }`));
  it('modifier vs @error -> both reject', () =>
    bothReject(`@contract class C { @error x(); @modifier x() { _; } }`, `contract C { error x(); modifier x() { _; } }`));
  it('modifier vs @function -> both reject', () =>
    bothReject(
      `@contract class C { @modifier x() { _; } @external @pure x(): u256 { return 1n; } }`,
  ...
  it('modifier vs @state declared AFTER the modifier -> both reject (order independent)', () =>
    bothReject(
      `@contract class C { @modifier x() { _; } @state x: u256; @external f(): u256 { return this.x; } }`,
  ...
  it('uses JETH133 for a same-contract modifier-vs-field clash', () =>
    expect(codes(`@contract class C { @state x: u256; @modifier x() { _; } }`)).toContain('JETH133'));
});
...
describe('W8B: @modifier cross-inheritance duplicate identifiers (both directions + diamond)', () => {
  it('base @modifier + derived @state -> both reject', () =>
    bothReject(
      `@abstract class B { @modifier x() { _; } } @contract class C extends B { @state x: u256; @external f(): u256 { return this.x; } }`,
  ...
  it('base @modifier + derived @event -> both reject', () =>
    bothReject(
      `@abstract class B { @modifier x() { _; } } @contract class C extends B { @event x(); }`,
      `abstract contract B { modifier x() { _; } } contract C is B { event x(); }`,
    ));
  it('base @modifier + derived @error -> both reject', () =>
    bothReject(
      `@abstract class B { @modifier x() { _; } } @contract class C extends B { @error x(); }`,
      `abstract contract B { modifier x() { _; } } contract C is B { error x(); }`,
    ));
  it('base @modifier + derived @function -> both reject', () =>
    bothReject(
      `@abstract class B { @modifier x() { _; } } @contract class C extends B { @external @pure x(): u256 { return 1n; } }`,
  ...
  it('base @state + derived @modifier -> both reject', () =>
    bothReject(
      `@abstract class B { @state x: u256; } @contract class C extends B { @modifier x() { _; } @external f(): u256 { return this.x; } }`,
  ...
  it('base @event + derived @modifier -> both reject', () =>
    bothReject(
      `@abstract class B { @event x(); } @contract class C extends B { @modifier x() { _; } }`,
      `abstract contract B { event x(); } contract C is B { modifier x() { _; } }`,
    ));
  it('base @function + derived @modifier -> both reject', () =>
    bothReject(
      `@abstract class B { @virtual @external @pure x(): u256 { return 1n; } } @contract class C extends B { @modifier x() { _; } }`,
  ...
  it('diamond: a base state var vs a derived modifier -> both reject', () =>
    bothReject(
      `@abstract class A { @state z: u256; } @abstract class B extends A { @state x: u256; } @contract class C extends B { @modifier x() { _; } @external f(): u256 { return this.x; } }`,
...
describe('W8B: @storage(ns) duplicate identifiers', () => {
  it('@storage(ns) x + @state x (same contract) -> reject (ambiguous this.x)', () =>
    expect(codes(`@contract class C { @storage('a.b') x: u256; @state x: u256; @external f(): u256 { return this.x; } }`)).toContain('JETH046'));
  it('@state x + @storage(ns) x (reversed order) -> reject', () =>
    expect(codes(`@contract class C { @state x: u256; @storage('a.b') x: u256; @external f(): u256 { return this.x; } }`)).toContain('JETH046'));
  it('@storage(ns) x + @constant x -> reject', () =>
    expect(codes(`@contract class C { @storage('a.b') x: u256; @constant x: u256 = 3n; }`)).toContain('JETH046'));
  it('@storage(ns) x + @modifier x -> reject (cross-kind JETH133)', () =>
    expect(codes(`@contract class C { @storage('a.b') x: u256; @modifier x() { _; } @external f(): u256 { return 1n; } }`)).toContain('JETH133'));
  it('two @storage fields, different namespaces, same name -> reject (ambiguous binding)', () =>
    expect(codes(`@contract class C { @storage('a') x: u256; @storage('b') x: u256; @external f(): u256 { return this.x; } }`)).toContain('JETH046'));
});

describe('W8B: legal reuse must stay ACCEPTED (no over-rejection)', () => {
  it('function overloading', () =>
    bothAccept(
      `@contract class C { @external @pure x(): u256 { return 1n; } @external @pure x(a: u256): u256 { return a; } }`,
  ...
  it('event overloading by signature', () =>
    bothAccept(`@contract class C { @event x(); @event x(a: u256); }`, `contract C { event x(); event x(uint256 a); }`));
  it('a modifier and a differently-named member', () =>
    bothAccept(
      `@contract class C { @state s: u256; @modifier onlyOwner() { _; } @external @onlyOwner set(v: u256): void { this.s = v; } }`,
  ...
  it('an inherited modifier with NO same-named derived member', () =>
    bothAccept(
      `@abstract class B { @modifier x() { _; } } @contract class C extends B { @state y: u256; }`,
  ...
  it('two @storage fields in different namespaces with DISTINCT names', () =>
    expect(codes(`@contract class C { @storage('a') p: u256; @storage('b') q: u256; @external f(): u256 { return this.p + this.q; } }`)).toEqual([]));
  it('a @storage field and a @state var with DISTINCT names', () =>
    expect(codes(`@contract class C { @storage('a.b') y: u256; @state x: u256; @external f(): u256 { return this.x + this.y; } }`)).toEqual([]));
});
