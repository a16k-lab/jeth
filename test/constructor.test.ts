...
describe('Phase 5 constructors vs solc 0.8.35', () => {
  ...
  describe('clean gates (each a clean diagnostic; no crash)', () => {
    it('a second constructor -> JETH300', () =>
      expect(
        codes(`@contract class C { @state x: u256; constructor(){this.x=1n;} constructor(a: u256){this.x=a;} }`),
      ).toContain('JETH300'));
    ...
    it('a default value on a constructor param -> JETH304', () =>
      expect(codes(`@contract class C { @state x: u256; constructor(a: u256 = 5n){ this.x = a; } }`)).toContain(
        'JETH304',
      ));
  });
  ...
  it('emits a Solidity-compatible constructor ABI entry', () => {
    const r = compile(
      `@contract class C { @state x: u256; @payable constructor(seed: u256, who: address) { this.x = seed; } }`,
      { fileName: 'C.jeth' },
    );
  ...
  it('a no-constructor contract is unaffected (still deploys + reads state)', async () => {
    ...
    const a = await h.deploy(
      compile(`@contract class C { @state x: u256 = 5n; @external @view gx(): u256 { return this.x; } }`, {
        fileName: 'C.jeth',
      }).creationBytecode,
    );
