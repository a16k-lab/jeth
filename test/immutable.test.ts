...
async function sameCall(J: string, S: string, sig: string, args = '', arg = '') {
  const j = await dJ(J, args), s = await dS(S, args);
...
async function sameSlots(J: string, S: string, args = '', n = 3) {
  ...
  for (let i = 0; i < n; i++) { js.push(await readSlot(j.h, j.a, BigInt(i))); ss.push(await readSlot(s.h, s.a, BigInt(i))); }
...
describe('Phase 5 immutables (@immutable) vs solc 0.8.35', () => {
  it('consumes NO storage slot: @state/@immutable/@state keeps slots unshifted', () =>
    sameSlots(
      ...
      `contract C { uint256 s0; uint256 immutable a; uint256 s1; constructor(){ s0=11; a=7; s1=22; } }`, '', 2));
  ...
  it('does not break packing or finish a slot (u8 / immutable u128 / u8 pack together)', () =>
    sameSlots(
      ...
      `contract C { uint8 x; uint128 immutable a; uint8 y; uint256 c; constructor(){ x=1; a=5; y=2; c=9; } }`, '', 2));

  it('bakes a constructor-arg immutable, read at runtime (-> 42)', async () => {
    expect(BigInt(await sameCall(
  ...
  it('two packed-width immutables from args; surrounding @state slot intact', async () => {
    expect(BigInt(await sameCall(
      ...
      'getB()', pad32(7n) + pad32(9n)))).toBe(9n);
    await sameSlots(
  ...
  describe('mutability classification (immutable read needs @view, not @pure)', () => {
    ...
    it('@pure reading an immutable is rejected (JETH164), matching solc', () =>
      expect(codes(`@contract class C { @immutable a: u256; @external @pure getA(): u256 { return this.a; } constructor(){ this.a = 1n; } }`)).toContain('JETH164'));
  });
  ...
  describe('clean gates', () => {
    ...
    it('an inline-initialized immutable -> JETH311', () =>
      expect(codes(`@contract class C { @immutable a: u256 = 7n; constructor(){} }`)).toContain('JETH311'));
    it('a @public immutable (auto-getter) -> JETH312', () =>
      expect(codes(`@contract class C { @public @immutable a: u256; constructor(){ this.a = 1n; } }`)).toContain('JETH312'));
    it('assigning an immutable outside the constructor -> JETH313', () =>
      expect(codes(`@contract class C { @immutable a: u256; @external setit(): void { this.a = 1n; } constructor(){} }`)).toContain('JETH313'));
    it('@state and @immutable on the same field -> JETH052', () =>
      expect(codes(`@contract class C { @state @immutable a: u256; constructor(){ this.a = 1n; } }`)).toContain('JETH052'));
  });

  it('immutables contribute NO ABI getter (only a constructor entry / the explicit view fn)', () => {
    const r = compile(`@contract class C { @immutable a: u256; @external @view getA(): u256 { return this.a; } constructor(x: u256){ this.a = x; } }`, { fileName: 'C.jeth' });
