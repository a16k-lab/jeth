...
describe('audit library/struct over-rejections lifted byte-identical', () => {
  ...
  it('OR8: a @state storage struct passed to an internal @library function', async () => {
    await eqCalls(
      'type S = { a: u256; b: u256; }; static class L { sm(s: S): u256 { return s.a * 10n + s.b; } } @using(L) class C { st: S; set(x: u256, y: u256): External<void> { this.st.a = x; this.st.b = y; } get go(): External<u256> { return L.sm(this.st); } }',
  ...
  it('OR8: a storage struct to a plain internal fn (storage / mapping / array element sources)', async () => {
    await eqCalls(
      'type S = { a: u256; b: u256; }; class C { st: S; m: mapping<u256,S>; arr: S[]; g(s: S): u256 { return s.a * 10n + s.b; } seed(): External<void> { this.st.a = 1n; this.st.b = 2n; this.m[5n].a = 3n; this.m[5n].b = 4n; this.arr.push(S(6n, 7n)); } get gst(): External<u256> { return this.g(this.st); } get gm(): External<u256> { return this.g(this.m[5n]); } get ga(): External<u256> { return this.g(this.arr[0n]); } }',
  ...
  it('OR8: packed storage struct + copy semantics (callee write does not affect storage)', async () => {
    await eqCalls(
      'type S = { a: u128; b: u128; }; class C { st: S; mut(s: S): u256 { s.a = 999n; return u256(s.a) + u256(s.b); } set(x: u128, y: u128): External<void> { this.st.a = x; this.st.b = y; } get go(): External<u256> { return this.mut(this.st); } get ra(): External<u128> { return this.st.a; } }',
  ...
  it('OR9: an @external (delegatecall) @library function that RETURNS a struct', async () => {
    const jeth = 'type P = { x: u256; y: u256; }; static class L { mk(a: u256): External<P> { return P(a, a + 1n); } } class C { go(a: u256): External<P> { return L.mk(a); } go2(a: u256): External<P> { let p: P = L.mk(a); return p; } }';
  ...
  it('OR9: return abi.decode(b, P) (a struct) is byte-identical incl malformed-input revert', async () => {
    const jeth = 'type P = { x: u256; y: u256; }; class C { get go(b: bytes): External<P> { return abi.decode(b, P); } }';
