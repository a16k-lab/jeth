...
describe('Constructor aggregate/dynamic params + internal calls vs solc 0.8.35 (JETH302/JETH303)', () => {
  ...
  it('u256[] param: decode + store length and xs[0] (raw slots identical)', () =>
    sameSlots(
      `class C { n: u256; first: u256; constructor(xs: u256[]) { this.n = xs.length; this.first = xs[0n]; } }`,
  ...
  it('bytes param: store length + a byte (raw slots identical)', () =>
    sameSlots(
      `class C { n: u256; b1: u256; constructor(b: bytes) { this.n = b.length; this.b1 = u256(u8(b[1n])); } }`,
  ...
  it('string param: store length (raw slots identical)', () =>
    sameSlots(
      `class C { n: u256; constructor(s: string) { this.n = bytes(s).length; } }`,
  ...
  it('value-struct param S{a;b}: store both fields (raw slots identical)', () =>
    sameSlots(
      `type S = { a: u256; b: address }; class C { x: u256; who: address; constructor(s: S) { this.x = s.a; this.who = s.b; } }`,
  ...
  it('fixed Arr<u256,3> param: store each element (raw slots identical)', () =>
    sameSlots(
      `class C { a0: u256; a1: u256; a2: u256; constructor(a: Arr<u256,3>) { this.a0 = a[0n]; this.a1 = a[1n]; this.a2 = a[2n]; } }`,
  ...
  it('dynamic-field struct param D{a;s:bytes}: store a + s.length + s[0] (raw slots identical)', () =>
    sameSlots(
      `type D = { a: u256; s: bytes }; class C { x: u256; n: u256; b0: u256; constructor(d: D) { this.x = d.a; this.n = d.s.length; this.b0 = u256(u8(d.s[0n])); } }`,
  ...
  it('ctor calling an internal helper (this.x = this.helper()) (raw slots identical)', () =>
    sameSlots(
      `class C { x: u256; helper(): u256 { return 41n + 1n; } constructor() { this.x = this.helper(); } }`,
  ...
  it('ctor calling a TRANSITIVE internal chain (helper calls helper2) (raw slots identical)', () =>
    sameSlots(
      `class C { x: u256; inner(): u256 { return 100n; } outer(): u256 { return this.inner() + 7n; } constructor() { this.x = this.outer(); } }`,
  ...
  it('ctor internal call that READS msg.sender / writes state (raw slots identical)', () =>
    sameSlots(
      `class C { owner: address; setup(): address { return msg.sender; } constructor() { this.owner = this.setup(); } }`,
  ...
  it('aggregate param + internal call combo (raw slots identical)', () =>
    sameSlots(
      `class C { x: u256; dbl(v: u256): u256 { return v * 2n; } constructor(xs: u256[]) { this.x = this.dbl(xs[0n]) + xs.length; } }`,
  ...
  it('internal helper TAKING an aggregate (memory array) param (raw slots identical)', () =>
    sameSlots(
      `class C { x: u256; sumFirstTwo(arr: u256[]): u256 { return arr[0n] + arr[1n]; } constructor(xs: u256[]) { this.x = this.sumFirstTwo(xs); } }`,
  ...
  it('SHORT ARGS (truncated u256[] header): both revert', () =>
    bothRevert(
      `class C { n: u256; constructor(xs: u256[]) { this.n = xs.length; } }`,
  ...
  it('SHORT ARGS (offset points past the blob): both revert', () =>
    bothRevert(
      `class C { n: u256; constructor(b: bytes) { this.n = b.length; } }`,
  ...
  it('SHORT ARGS (array length present, element words missing): both revert', () =>
    bothRevert(
      `class C { n: u256; f: u256; constructor(xs: u256[]) { this.n = xs.length; this.f = xs[0n]; } }`,
