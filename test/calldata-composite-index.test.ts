...
describe('calldata composite element access (JETH151/210) vs Solidity', () => {
  ...
  it('dynamic-of-fixed uint256[2][] a[i][j] (+ OOB)', async () => {
    ...
    const head = (i: bigint, j: bigint) =>
      '0x' + sel('dof(uint256[2][],uint256,uint256)') + pad(0x60n) + pad(i) + pad(j) + encDof(rows);
  ...
  it('fixed-of-dynamic uint256[][2] a[i][j] + a[i].length (+ OOB)', async () => {
    ...
    const fod = (i: bigint, j: bigint) =>
      '0x' + sel('fod(uint256[][2],uint256,uint256)') + pad(0x60n) + pad(i) + pad(j) + encFod(rows);
