...
describe('native receive / fallback (a method named receive/fallback = the special entry)', () => {
  it('bare receive/fallback are byte-identical to the @receive/@fallback form', () => {
    ...
    expect(bc(`class C ${rBody.replace('#KW#', 'receive')}`)).toBe(bc(`@contract class C ${rBody.replace('#KW#', '@receive receive')}`));
    expect(bc(`class C ${rBody.replace('#KW#', 'fallback')}`)).toBe(bc(`@contract class C ${rBody.replace('#KW#', '@fallback fallback')}`));
    // the footgun: a bare receive is now the ENTRY (differs from a contract with no receive at all).
    expect(bc(`class C { @state x: u256; receive(): void { this.x = 1n; } @external @view g(): u256 { return this.x; } }`))
      .not.toBe(bc(`class C { @state x: u256; @external @view g(): u256 { return this.x; } }`));
    // @payable fallback + the data-passing fallback are also byte-identical.
    expect(bc(`class C { @payable fallback(): void {} }`)).toBe(bc(`@contract class C { @payable @fallback fallback(): void {} }`));
    expect(bc(`class C { fallback(input: bytes): bytes { return input; } }`)).toBe(bc(`@contract class C { @fallback fallback(input: bytes): bytes { return input; } }`));
  });

  it('a native receive accepts ether byte-identical to solc `receive() external payable`', async () => {
    const J = `class C { @state got: u256; receive(): void { this.got = msg.value; } @external @view seen(): u256 { return this.got; } }`;
  ...
  it('a native fallback runs on an unknown selector byte-identical to solc', async () => {
    const J = `class C { @state hit: u256; fallback(): void { this.hit = 7n; } @external @view seen(): u256 { return this.hit; } }`;
  ...
  it('rejects a wrong shape or a duplicate (receive is a reserved special entry)', () => {
    expect(codes(`class C { receive(x: u256): void {} @external @view g(): u256 { return 1n; } }`)).toContain('JETH384'); // receive takes no params
    expect(codes(`class C { receive(): u256 { return 1n; } @external @view g(): u256 { return 1n; } }`)).toContain('JETH384'); // receive returns nothing
    expect(codes(`class C { receive(): void {} @receive r2(): void {} }`)).toContain('JETH383'); // at most one receive
