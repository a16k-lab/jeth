  ...
  it('calldata array / dynamic-struct components (various sizes)', async () => {
    for (const xs of [[], [1n], [1n, 2n, 3n], [M - 1n, 0n, 42n]] as const) {
      ...
      await eq(
        `two([${xs.length}])`,
        '0x' +
          sel('two(bytes,uint256[])') +
          pad(0x40n) +
          pad(BigInt(0x40 + encStr('payload').length / 2)) +
          encStr('payload') +
          encU256Arr([...xs]),
      );
    }
    for (const ss of [
      [],
      ['a'],
      ['short', 'a much longer string element exceeding thirty-two bytes here ok'],
      ['', ''],
    ] as const) {
    ...
    for (const [a, s] of [
      [7n, 'hi'],
      [M - 1n, 'a longer string field over thirty-two bytes for the struct'],
      [0n, ''],
    ] as const) {
