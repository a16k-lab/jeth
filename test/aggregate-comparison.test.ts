...
// test pins the gate: aggregates reject JETH088; every value type still compares fine.
...
describe('aggregate comparison gate (JETH088, solc parity)', () => {
  ...
  it('rejects == on a dynamic array, a fixed array, bytes, and string', () => {
    ...
    expect(codes(fn('let a: bytes = bytes(""); let b: bytes = bytes(""); return a == b;'))).toContain('JETH088');
    expect(codes(fn('let a: string = "x"; let b: string = "y"; return a == b;'))).toContain('JETH088');
  });
